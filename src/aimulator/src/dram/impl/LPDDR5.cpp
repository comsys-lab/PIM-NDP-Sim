...
class LPDDR5 : public IDRAM, public Implementation {
  ...
  private:
    ...
    void set_preqs() {
      ...
      // Bank Preqs
      m_preqs[m_levels["bank"]][m_commands["RD16"]] = [] (Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
        switch (node->m_state) {
      ...
      m_preqs[m_levels["bank"]][m_commands["WR16"]] = [] (Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
        switch (node->m_state) {
          case m_states["Closed"]: return m_commands["ACT-1"];
          case m_states["Pre-Opened"]: return m_commands["ACT-2"];
          case m_states["Opened"]: {
