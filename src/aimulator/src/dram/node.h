...
struct DRAMNodeBase {
  ...
  void update_timing(int command, const AddrVec_t& addr_vec, Clk_t clk) {
     ...
     ***********************************************/
    // Determine whether the node is sibling
    if (m_node_id != addr_vec[m_level] && addr_vec[m_level] != -1) {
      // if the node is sibling, check the all timing constraints at this level.
    ...
    if (m_cmd_history[command].size()) {
      ...
      // Add current clock to front.
      m_cmd_history[command].push_front(clk); 
    }

    for (const auto& t : m_spec->m_timing_cons[m_level][command]) {
      if (t.sibling) {
        // Skip sibling timing constraints since we have already handled.
        continue;
      }
