...
namespace Bank {
  ...
  template <class T>
  void RD(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void WR(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void RVRR(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
...
namespace Rank {
  ...
  template <class T>
  void PREA(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void RFMsb(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void RFMsb_end(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void RRFMsb(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
  ...
  template <class T>
  void RRFMsb_end(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
    ...
    for (auto bankgroup_node : node->m_child_nodes) {
      for (auto bank_node : bankgroup_node->m_child_nodes) {
        if (bank_node->m_state == T::m_states["Opened"]) {
          if (bank_node->m_node_id == addr_h[T::m_levels["bank"]]) {
            open_target_banks++;
          } else {
  ...
  template <class T>
  void finalize_rank(typename T::Node* node, int cmd, const AddrHierarchy_t& addr_h, Clk_t clk) {
