...
class IDRAM : public Clocked<IDRAM> {
  ...
  public:
    ...
    SpecLUT<Level_t> m_command_addressing_level{m_commands};        // A LUT of the scopes (i.e., at which organization level) of the DRAM commands
    ...
    // AiM
    SpecDef m_aim_req;
    SpecLUT<Command_t> m_aim_req_translation{m_aim_req};
    SpecLUT<Level_t> m_command_action_scope{m_commands};
  ...
  public:
     ...
     * 
     */
    virtual void issue_command(int command, const AddrHierarchy_t& addr_h) = 0;
     ...
     * 
     */
    virtual int get_preq_command(int command, const AddrHierarchy_t& addr_h) = 0;
     ...
     * 
     */
    virtual bool check_ready(int command, const AddrHierarchy_t& addr_h) = 0;
     ...
     * 
     */
    virtual bool check_rowbuffer_hit(int command, const AddrHierarchy_t& addr_h) = 0;

    /**
     * @brief     
     * @details
     */
    virtual bool check_node_open(int command, const AddrHierarchy_t& addr_h) = 0;
...
#define RAMULATOR_DECLARE_SPECS() \
  ...
  IDRAM::m_command_addressing_level = m_command_addressing_level; \
  ...
  IDRAM::m_aim_req = m_aim_req; \
  IDRAM::m_aim_req_translation = m_aim_req_translation; \
  IDRAM::m_command_action_scope = m_command_action_scope; \
