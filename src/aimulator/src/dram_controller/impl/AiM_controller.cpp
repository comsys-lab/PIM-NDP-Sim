...
class AiMController final : public IDRAMController, public Implementation {
  ...
  public:
    ...
    bool send(Request& req) override {
      if (!req.is_aim_req) {
        if (m_aim_bank_buffer.size() != 0) { return false; }
        req.final_command = m_dram->m_request_translations(req.type_id);
      } else {
        if (m_write_buffer.size() != 0 || m_read_buffer.size() != 0) { return false; }
        // RD/WR excluded.
    ...
    void tick() override {
      ...
      // std::cout << "request found? " << request_found << std::endl;
      ...
      if (pending_reads.size()) {
        ...
        if (req.depart <= m_clk) {
          ...
          if (req.callback) {
            ...
            std::cout << "[AiM Controller] callback request type: " << req.command << " addr: " << req.addr << std::endl;
            req.callback(req);
          }
        ...
        while (write_req_it != pending_writes.end()) {
          if (write_req_it->depart <= m_clk) {
            if(write_req_it->callback){
              std::cout << "[AiM Controller] callback request type: " << write_req_it->command << " addr: " << write_req_it->addr << std::endl;
      ...
      if (!request_found) {
        ...
        if (!request_found) {
          if (m_aim_bank_buffer.size() != 0) {
            ...
            req_it->command = m_dram->get_preq_command(req_it->final_command, req_it->addr_h);
            std::cout << "[Ramulator (AiM Controller)] 2.2.2 aim preq cmd: " << req_it->command << std::endl;
            request_found = m_dram->check_ready(req_it->command, req_it->addr_h);
            std::cout << "[Ramulator (AiM Controller)] 2.2.2 ready?: " << request_found << std::endl;
