  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      if (m_dram->m_command_meta(req_it->command).is_closing ||
        ...
        // PRE or REF 
        if (req_it->addr_h[m_bankgroup_level] == -1 && req_it->addr_h[m_bank_level] == -1) {  // all bank closes
          for (int b = 0; b < m_num_banks; b++) {
            for (int bg = 0; bg < m_num_bankgroups; bg++) {
              int rank_id = req_it->addr_h[m_rank_level];
              int flat_bank_id = b + bg * m_num_banks + rank_id * m_num_banks * m_num_bankgroups;
              m_col_accesses[flat_bank_id] = 0;
            }
          }
        } else if (req_it->addr_h[m_bankgroup_level] == -1) {  // same bank closes
          for (int bg = 0; bg < m_num_bankgroups; bg++) {
            int bank_id = req_it->addr_h[m_bank_level];
            int rank_id = req_it->addr_h[m_rank_level];
          ...
          int flat_bank_id = req_it->addr_h[m_bank_level] + 
                             req_it->addr_h[m_bankgroup_level] * m_num_banks + 
                             req_it->addr_h[m_rank_level] * m_num_banks * m_num_bankgroups;
        ...
        if (m_col_accesses[flat_bank_id] >= m_cap) {
          Request req(req_it->addr_h, m_PRE_req_id);
