...
class BlockHammer : public IControllerPlugin, public Implementation, public IBlockHammer {
  ...
  private:
    ...
    BaseFilter* get_bank_filter(Request& req) {
      int flat_bank_id = req.addr_h[m_bank_level];
      ...
      for (int i = m_bank_level - 1; i >= m_rank_level; i--) {
        ...
        flat_bank_id += req.addr_h[i] * accumulated_dimension;
      }
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      bool is_row = m_dram->m_command_addressing_level(req_it->command) == m_row_level; 
    ...
    bool is_act_safe(Request& req) {
      ...
      bool is_row = m_dram->m_command_addressing_level(req.command) == m_row_level; 
      if (is_opening && is_row) {
        auto rank_idx = req.addr_h[m_rank_level];
        auto row_addr = req.addr_h[m_row_level];
