...
class CounterBasedTRR : public IControllerPlugin, public Implementation {
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) {
      ...
      if (request_found) {
        int rank_id = req_it->addr_h[m_rank_level_idx];
        if (req_it->command == m_ACT_id) {
          int bank_id = -1;
          if (m_bankgroup_level_idx == -1) {
            bank_id = req_it->addr_h[m_bank_level_idx];
          } else {
            int bank_groupid = req_it->addr_h[m_bankgroup_level_idx];
            bank_id = bank_groupid * m_num_banks_per_bankgroup + req_it->addr_h[m_bank_level_idx];
          }
          int row_addr = req_it->addr_h[m_row_level_idx];
