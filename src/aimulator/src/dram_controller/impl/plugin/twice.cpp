...
class TWiCeIdeal : public IControllerPlugin, public Implementation {
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      if (request_found) {
        if (m_dram->m_command_meta(req_it->command).is_refreshing && m_dram->m_command_addressing_level(req_it->command) == m_rank_level) {
          // Refresh command
        ...
        } else if (m_dram->m_command_meta(req_it->command).is_opening && m_dram->m_command_addressing_level(req_it->command) == m_row_level) {
          // Activation command
          ...
          int row_id = req_it->addr_h[m_row_level];

          if (m_is_debug) {
            std::cout << "TWiCeIdeal: ACT on row " << row_id << std::endl;
            std::cout << "  └  " << "rank: " << req_it->addr_h[m_rank_level] << std::endl;
            std::cout << "  └  " << "bank_group: " << req_it->addr_h[m_rank_level + 1] << std::endl;
            std::cout << "  └  " << "bank: " << req_it->addr_h[m_bank_level] << std::endl;
