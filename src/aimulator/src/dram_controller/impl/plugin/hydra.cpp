...
class Hydra : public IControllerPlugin, public Implementation {
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      if (request_found){
        if (m_dram->m_command_meta(req_it->command).is_opening && m_dram->m_command_addressing_level(req_it->command) == m_row_level){
          int flat_bank_id = req_it->addr_h[m_bank_level];
          ...
          for (int i = m_bank_level - 1; i >= m_rank_level; i--) {
            ...
            flat_bank_id += req_it->addr_h[i] * accumulated_dimension;
          }
          
          uint rank_id = req_it->addr_h[m_rank_level];
          uint bank_id = flat_bank_id % m_num_banks_per_rank;
          uint row_id = req_it->addr_h[m_row_level];
          ...
          if (row_id < m_total_rct_row_size){
            ...
            if (rct_count_table[flat_bank_id][row_id] >= m_tracking_threshold){
              ...
              // issue VRR
              Request vrr_req(req_it->addr_h, m_VRR_req_id);
          ...
          if (group_count_table[flat_bank_id][gct_index].group_count >= m_group_threshold){
            ...
            if (!group_count_table[flat_bank_id][gct_index].initialized){
              ...
              for (int i = 0; i < m_group_rct_cl_size; i++){
                ...
                for (int j = 0; j < req_it->addr_h.size(); j++){
                  rct_init_addr_vec.push_back(req_it->addr_h[j]);
                }
            ...
            if (row_count_cache[rank_id][rcc_index].find(rcc_tag) == row_count_cache[rank_id][rcc_index].end()){
              ...
              if (row_count_cache[rank_id][rcc_index].size() == 16){
                ...
                for (int i = 0; i < req_it->addr_h.size(); i++){
                  evicted_entry_addr_vec.push_back(req_it->addr_h[i]);
                }
              ...
              AddrHierarchy_t rct_read_addr_vec;
              for (int i = 0; i < req_it->addr_h.size(); i++){
                rct_read_addr_vec.push_back(req_it->addr_h[i]);
              }
    ...
    void reserve_rows_for_rct() {
      ...
      for (Addr_t addr = 0; addr < max_addr; addr += 64) {
        ...
        Addr_t row_id = req.addr_h[m_row_level];
        if (row_id < m_total_rct_row_size){
